...
- [x] **Çıkma (salvage) ürünleri için web tabanlı araç uyumu araması**: `searchForSalvageReferences(specs, input, imagePath)` (YENİ) — sadece `condition === 'salvage'`'de çalışır; visibleText'ten döküm/etiket yazılarını, tüm kod değerlerini, parça tipinin farklı adlarını ve araç kategorisi ipuçlarını (kamyon/kamyonet/binek vb.) sorgulara dönüştürür (`"<kod>"`, `"<kod> çıkma parça"`, `"<marka> <kod> araç uyumu"`, `"<marka> <kod> söküm"`, `"<visibleText satırı>"`, `"<visibleText satırı> çıkma parça"`, `"<marka> <tür> çıkma"`, `"<marka> <tür> uyumlu araç"`, `"<tür> çıkma parça"`, `"<marka> <araç_kategorisi>"` vb., maks. 15 sorgu). **Google Cloud Vision API (`WEB_DETECTION`)** ile görsel arama (opsiyonel, `GOOGLE_CLOUD_VISION_API_KEY` gerekli): fotoğraf base64 olarak GCV'ye gönderilir, `bestGuessLabels` + `webEntities` + `pagesWithMatchingImages` sonuçları metin aramasıyla birleştirilir. Sonuçlar "Salvage Reference Search Results" olarak prompt'a enjekte edilir. System prompt SALVAGE RULES: visibleText ve observations'dan araç markası/tipi/serisi çıkarılması istenir; başlıkta marka + parça adı + araç modeli + "(Çıkma)" zorunlu; açıklama "Uyumlu Araçlar" (tüm uyumlu araçlar, marka+model+seri+yıl), "Kullanım Yeri", "Söküm Bilgisi", "Montaj Notları", "Araç Tipi" bölümleri içermeli; SEO meta title/description araç modeli + parça + "çıkma" içermeli; keyword'ler "<marka> <parça> çıkma", "<araç modeli> <parça> uyumlu", "<parça kodu>", "<marka> <parça> söküm", "<araç modeli> <parça> çıkma" içermeli; uydurma araç uyumu yasak (sadece reference sonuçlarından veya visibleText/observations'dan); attributes "Uyumlu Araçlar", "Kullanım Yeri", "Söküm Bilgisi", "Araç Tipi" içermeli. `generateAgenticListing` salvage'de `searchForCodes` yerine `searchForSalvageReferences` çağırır; diğer durumlarda (`new`/`refurbished`/`used`) mevcut `searchForCodes` korunur.
- [x] **Doğrulamalar** — ai-service typecheck ✅ + test **14/14** ✅ (GCV mock, salvage branching, visibleText+araç ipuçları, search hata dayanıklılığı), core typecheck ✅ + test 54/54 ✅, frontend build ✅ (48 route), mobil `npx tsc --noEmit` ✅.

### Çıkma (Salvage) AI Geliştirmeleri — Araç Uyumu + GCV + Satıcı Notu ✅
Tüm commit zinciri: `8fc5b50` (salvage function) → `40219a1` (GCV birincil kaynak) → `a83efa9` → `c61aa03` → `10ab3a2` → `8b6653d` → `eabfdb2` → `45e00f9` (revert) → `a4cf178` (polling+paralel arama) → `2d6ef74` (satıcı notu + bütün araç + materyal yasağı).

- [x] **Salvage (çıkma) görsel analizi birincil kaynak** (`40219a1`): `condition === 'salvage'`'de `analyzeSalvageVehicle` GCV tam görsel analizi (`webSearch.ts` `analyzeProductImageFullGCV`) birincil kaynak; görselden araç markası/modeli/parça adı + uyumlu araçlar çıkarılır (`brand`, `model`, `partName`, `compatibleVehicles`).
- [x] **GCV uyumlu araç listesi** (`a83efa9`, `c61aa03`): GCV `WEB_DETECTION` `pagesWithMatchingImages` sayfa içeriklerinden uyumlu araçlar çıkarılır → `compatibleVehicles[]` → prompt'a "Compatible Vehicles" olarak net enjekte edilir; başlıkta en az bir uyumlu marka/model zorunlu, "Uyumlu Araçlar" attribute'u tüm listeyi içermeli.
- [x] **Numerik parça kodu + marka refine** (`10ab3a2`): `parseGcvCodes` ile görseldeki numerik parça kodları tespit edilir; GCV guess generic ise reference sonuçlarından marka refine edilir.
- [x] **Multimodal marka/model tespiti** (`8b6653d`): GCV generic sonuç dönerse multimodal vision (Gemini) ile araç marka/model tespiti (`visionAnalyzer.ts`).
- [x] **İlan numarası/watermark engeli** (`eabfdb2` + `45e00f9` revert): Google CSE (`SEARCH_API_KEY`)/Bing fallback eklenmişti ama Google "tüm web" desteği kapattığı için geri alındı (artık sadece DuckDuckGo + GCV). `parseGcvCodes` 8+ haneli ad numaralarını part_code olarak prompt'a sokmaz; SALVAGE_PROMPT'ta ad-numarası kuralı + marka ayrımı korunur.
- [x] **Draft polling + paralel arama** (`a4cf178`): AI Studio 'Taslak hazırlanamadı' fix'i — draft polling 120sn'ye uzatıldı + salvage web araması paralel çalışır.
- [x] **Satıcı notu otoriter (AUTHORITATIVE)** (`2d6ef74`): `input.shortDescription` artık hem `analyzeProductImage` hem `analyzeSalvageVehicle`'a geçirilir (visionAnalyzer `sellerNote` parametresi, user mesajına enjekte edilir). Listing prompt'unda SELLER NOTE RULE: satıcı notu (örn. "boş kupa", "kabin", parça kodu) **otoriterdir** — fotoğraf komple araç gösterse bile ürün satıcının tarif ettiği parçadır; başlık + açıklama satıcının belirttiği ürüne göre yazılır.
- [x] **Bütün araç (WHOLE VEHICLE) modu** (`2d6ef74`): `isWholeVehicle` tespiti — satıcı notunda "tüm parça/tüm parçalar/söküm/parça için/komple araç/komple kamyonet/komple kamyon/tam komple/sökülüyor/parçalanıyor/parçalanmış/parça amaçlı/hurda araç/komple/parçalar halinde" sinyalleri. Bu modda ürün tek parça DEĞİL, bütün araç parçalara ayrılıyor: başlık `"BRAND + Model + Araç Tipi (Çıkma)"` (örn. "BMC Pro Kamyonet (Çıkma)"), açıklamada "Araçtan Sökülen Parçalar" bölümü parçaları **tek tek** listeler (kabin, kapılar, kaput, tampon, stop lambaları, aynalar, şanzıman, yürür aksam, döşeme vb.), "Kullanım Yeri" yerine "Parça Durumu", görünmeyen parça uydurma yasak. Spesifik parça notu (örn. "boş kupa") bütün araç modunu ezmez.
- [x] **Materyal yasağı (MATERIAL RULE)** (`2d6ef74`): çıkma ürünlerde materyal (metal, plastik, çelik, alüminyum, kumaş, döşeme, deri vb.) başlık/açıklama/keyword/attribute'ların **hiçbirinde** geçmez; vision prompt'u da materyal raporlamaz; listing şemasında `"Materyal": ""` boş kalır.
- [x] **Mobil kısa açıklama alanı** (`2d6ef74`): `mobile-app/app/(tabs)/ai.tsx` analiz öncesi "Kısa Açıklama (opsiyonel)" input (placeholder: "Ürünü tek satırda anlat (örn. boş kupa, BMC pro kabin)", maxLength 300); `mobile-app/src/shared/api-client.ts` `createAiProductSessionFromImage` opts.`shortDescription` → `short_description` passthrough. Web Studio `input.shortDescription` zaten mevcuttu.
- [x] **Doğrulamalar** (`2d6ef74`): ai-service typecheck ✅ + test **21/21** ✅ (yeni: satıcı notu otoriter, bütün araç modu, spesifik parça override, Materyal boş), core test 54/54 ✅, frontend build ✅, mobil `npx tsc --noEmit` ✅. Production'a deploy edildi (git tabanlı stack rebuild, ConfigHash `2d6ef74`).
