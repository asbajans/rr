...
- [x] **Çıkma (salvage) ürünleri için web tabanlı araç uyumu araması**: `searchForSalvageReferences(specs, input)` (YENİ) — sadece `condition === 'salvage'`'de çalışır; çoklu sorgu üretir (`"<kod>"`, `"<kod> çıkma parça"`, `"<kod> hangi araca uyumlu"`, `"<marka> <kod> araç uyumu"`, `"<marka> <kod> söküm"`, `"<marka> <tür> çıkma"`, `"<marka> <kategori> parça uyumlu araç"`). Sonuçlar "Salvage Reference Search Results" olarak prompt'a enjekte edilir. System prompt SALVAGE RULES: başlıkta marka + parça adı + araç modeli + "(Çıkma)" zorunlu; açıklama "Uyumlu Araçlar", "Kullanım Yeri", "Söküm Bilgisi", "Montaj Notları" bölümleri içermeli; SEO meta title/description araç modeli + parça + "çıkma" içermeli; keyword'ler "<marka> <parça> çıkma", "<araç modeli> <parça> uyumlu", "<parça kodu>", "<marka> <parça> söküm" içermeli; uydurma araç uyumu yasak (sadece reference sonuçlarından). `generateAgenticListing` salvage'de `searchForCodes` yerine `searchForSalvageReferences` çağırır; diğer durumlarda (`new`/`refurbished`/`used`) mevcut `searchForCodes` korunur.
- [x] **Doğrulamalar** — ai-service typecheck ✅ + test **11/11** ✅ (salvage arama sorguları, condition-based branching, search hata dayanıklılığı), core typecheck ✅ + test 54/54 ✅, frontend build ✅ (48 route), mobil `npx tsc --noEmit` ✅.
